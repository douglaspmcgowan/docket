...
function Resolve-WorkScopeNativeExecutable {
    ...
    if ([System.IO.Path]::GetExtension($source) -ne '.exe') {
        throw "Verification executable '$source' is not a native .exe. Batch files and shell scripts are rejected; invoke a fixed script through an explicitly declared native interpreter such as pwsh.exe."
    }
    return $source
}
...
function Invoke-WorkScopeVerification {
    ...
    $recordItem = Get-Item -LiteralPath $temporaryRecordPath
...
function Add-WorkScopeEvent {
    ...
    $candidateValidation = Test-WorkScopeState -State $State
...
function Repair-WorkScopeTransaction {
    ...
    if ($transaction.version -ne '1.0.0' -or
        [System.IO.Path]::GetFullPath([string]$transaction.state.project.root).TrimEnd('\', '/') -ne $rootFull) {
        throw "Transaction journal '$transactionPath' does not match this project."
    }
    $transactionValidation = Test-WorkScopeState -State ([hashtable]$transaction.state)
    ...
    if ($transaction.ContainsKey('file_commit') -and $null -ne $transaction.file_commit) {
        ...
        switch ($transaction.event.type) {
            ...
            'verification_executed' {
                ...
                $recordItem = Get-Item -LiteralPath $recordCandidate
...
function Test-WorkScopeState {
    ...
    if ($PSCmdlet.ParameterSetName -eq 'Root') {
        try {
            ...
            if ($null -eq $declaredRoot -or
                ...
                return [pscustomobject]@{
                    valid = $false
                    errors = @('Work Scope state is location-bound to its canonical project.root and does not match -Root. A relocated project requires deliberate reviewed re-enrollment; project.root is never repaired automatically.')
                }
            }
        ...
        try {
            ...
            if ($State.ContainsKey('project') -and $State.project -is [System.Collections.IDictionary] -and
                $State.project.Contains('root')) {
                $candidateSchema = Join-Path ([string]$State.project.root) '.agents\work\schema.json'
    ...
    if ($State.active.status -eq 'closed') {
        ...
        if (-not (Test-StructuredEvidence -Evidence $State.active.evidence -Root $State.project.root -RequireProvenance)) {
    ...
    foreach ($task in @($State.active.tasks)) {
        ...
        foreach ($check in $acceptanceChecks) {
            ...
            try {
                ...
                if ([System.IO.Path]::GetExtension([string]$check.executable) -ne '.exe') {
                    $errors.Add("Task '$($task.id)' acceptance check '$($check.id)' must use a native .exe.")
                }
                foreach ($artifact in @($check.artifacts)) {
                    Resolve-WorkScopeArtifact -Root $State.project.root -Artifact ([string]$artifact) | Out-Null
                }
                foreach ($inputSnapshot in @($check.verifier_inputs)) {
                    $inputReference = Resolve-WorkScopeArtifact -Root $State.project.root -Artifact ([string]$inputSnapshot.reference)
                    $inputPath = Join-Path $State.project.root $inputReference
                    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
                ...
                if ([System.IO.Path]::GetFileName([string]$check.executable) -in @('pwsh.exe', 'powershell.exe')) {
                    ...
                    if ($fileArgumentIndex -ge 0) {
                        if ($fileArgumentIndex + 1 -ge $declaredArguments.Count) {
                            ...
                            $scriptReference = Resolve-WorkScopeArtifact -Root $State.project.root -Artifact $declaredArguments[$fileArgumentIndex + 1]
        ...
        if (@('ready', 'blocked', 'closed') -notcontains $task.status) {
        ...
        if ($task.status -eq 'closed') {
            if (-not (Test-StructuredEvidence -Evidence $task.evidence -Root $State.project.root -RequireProvenance)) {
    ...
    foreach ($session in @($State.ownership.sessions)) {
        foreach ($artifact in @($session.artifacts)) {
            try {
                $normalizedArtifact = Resolve-WorkScopeArtifact -Root $State.project.root -Artifact $artifact
    ...
    foreach ($item in @($State.backburner)) {
        ...
        if (-not (Test-StructuredEvidence -Evidence $item.evidence -Root $State.project.root)) {
...
function Add-WorkScopeTask {
    ...
    if ([System.IO.Path]::GetFileName($checkExecutablePath) -in @('pwsh.exe', 'powershell.exe')) {
        $fileArgumentIndex = -1
...
function Close-WorkScopeCell {
    ...
    $open = @($state.active.tasks | Where-Object { $_.status -ne 'closed' })
    if ($open.Count -gt 0) {
...
function Import-WorkScopeLegacyRecords {
    ...
    foreach ($inputRecord in @($Records)) {
        ...
        $sourceItem = Get-Item -LiteralPath $absoluteSourcePath
    ...
    foreach ($source in $sourceFacts.Values) {
        if (Test-Path -LiteralPath $source.snapshot_path -PathType Leaf) {
            $snapshotItem = Get-Item -LiteralPath $source.snapshot_path
    ...
    try {
        ...
        foreach ($staleTemporary in @(Get-ChildItem -LiteralPath $importsRoot -File -Filter '.legacy-*.tmp')) {
        ...
        foreach ($source in $sourceFacts.Values) {
            if (-not (Test-Path -LiteralPath $source.snapshot_path)) {
                ...
                $temporaryItem = Get-Item -LiteralPath $temporarySnapshot
                ...
                if (Test-Path -LiteralPath $source.snapshot_path) {
                    $existingItem = Get-Item -LiteralPath $source.snapshot_path
...
`$($State.active.cell_id)` in track `$($State.active.track_id)`.
"@
