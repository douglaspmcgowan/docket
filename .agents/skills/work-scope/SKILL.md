...
name: work-scope
description: Control project work as one explicit capability-by-depth scope cell, with authoritative structured state, ownership guards, evidence-gated closure, bounded drilldown or expansion, durable discovery capture, resumable state, and independent-track handoffs. Use when starting, resuming, scoping, expanding, drilling down, handing off, or recording adjacent work in a project that contains .agents/work/state.json or is adopting the work-scope system.
---
...
Enrolled state is location-bound to its canonical absolute project root. A moved or copied project must fail validation until it is deliberately re-enrolled or reinitialized through a reviewed migration. Never edit or automatically repair `project.root` to make relocated state appear valid.

## Locate tools

Resolve the work-scope package root two directories above this `SKILL.md`, then
use that package root's `tools/` directory. Pass the target project path through
`-Root`. Never edit generated views directly.
