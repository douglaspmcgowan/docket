...
|---|---|---|---|---|---|---|---|
| `APP_SECRET` | Cloud UI and API bearer credential | Bitwarden Secrets Manager and Vercel | cloud runtime | Douglas | on compromise, access change, or planned credential rotation | api/_auth.js | credential-required |
...
| `BWS_ACCESS_TOKEN` | Read-only Secrets Manager machine-account bootstrap credential | Bitwarden Secrets Manager | credential broker parent | Douglas | use a narrow scope and expiration; rotate on compromise or machine-account change | Invoke-WithBitwardenSecret.ps1 | credential-required |
